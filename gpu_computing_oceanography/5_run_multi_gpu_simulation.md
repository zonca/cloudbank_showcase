...
\`\`\`bash
gcloud compute ssh julia-ocean-4gpu-hi --zone=us-west1-b
\`\`\`
...
**Expected Output:** You should see a table listing **4 NVIDIA Tesla T4** GPUs. 
...
\`\`\`bash
# We disable XLA memory pre-allocation to ensure multiple ranks don't fight for VRAM during compilation.
